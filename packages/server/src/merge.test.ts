...
test("prepareLocalElementsForMerge bumps hand-edited elements with stale version", () => {
  ...
  const prepared = prepareLocalElementsForMerge(local, parent) as Array<
    Record<string, unknown>
  >;
  ...
  assert.equal(
    byId.api!.versionNonce,
    deterministicVersionNonce("api", 2),
  );
...
test("prepareLocalElementsForMerge injects soft-delete for hard deletes", () => {
  ...
  assert.equal(
    byId.gone!.versionNonce,
    deterministicVersionNonce("gone", 3),
  );
